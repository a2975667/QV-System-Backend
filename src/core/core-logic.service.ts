...
export class CoreLogicService {
  ...
  mergeIdListWithDocList(
    ...
    console.log('[DEBUG] mergeIdListWithDocList - ID list length:', idList?.length);
    console.log('[DEBUG] mergeIdListWithDocList - Doc list length:', docList?.length);
    
    if (!idList || !Array.isArray(idList) || idList.length === 0) {
      console.log('[DEBUG] mergeIdListWithDocList - Empty or invalid ID list, returning empty array');
      return [];
    }
    
    if (!docList || !Array.isArray(docList) || docList.length === 0) {
      console.log('[DEBUG] mergeIdListWithDocList - Empty or invalid document list, returning empty array');
      return [];
    }
    
    const mergedDoc = idList.map((id, index) => {
      // Skip invalid IDs
      if (!id) {
        console.log(`[DEBUG] mergeIdListWithDocList - Skipping invalid ID at index ${index}`);
        return undefined;
      }
      
      console.log(`[DEBUG] mergeIdListWithDocList - Processing ID: ${id.toString()} (index ${index})`);
      ...
      if (!foundDoc) {
        console.log(`[DEBUG] mergeIdListWithDocList - No document found for ID: ${id.toString()}`);
        return undefined;
      }
      
      console.log(`[DEBUG] mergeIdListWithDocList - Found document for ID: ${id.toString()}`);
      ...
      if (foundDoc._doc) {
        ...
        console.log(`[DEBUG] mergeIdListWithDocList - Document is a Mongoose document with _doc`);
      } else {
        // Plain object
        docCopy = {...foundDoc};
        console.log(`[DEBUG] mergeIdListWithDocList - Document is a plain object`);
      }
      
      // Check for options
      if (docCopy.options) {
        console.log(`[DEBUG] mergeIdListWithDocList - Document has ${docCopy.options.length} options`);
      } else {
        console.log(`[DEBUG] mergeIdListWithDocList - Document has no options`);
      }
      ...
      if (removeField && Array.isArray(removeField)) {
        removeField.forEach((field) => {
          if (field && docCopy) {
            ...
            console.log(`[DEBUG] mergeIdListWithDocList - Removed field: ${field}`);
          }
        });
      }
    ...
    console.log(`[DEBUG] mergeIdListWithDocList - Completed merge, final length: ${mergedDoc.length}`);
    
    // Check the first merged doc for debugging
    if (mergedDoc.length > 0) {
      const firstDoc = mergedDoc[0];
      console.log(`[DEBUG] mergeIdListWithDocList - Sample merged doc ID: ${firstDoc._id.toString()}`);
      console.log(`[DEBUG] mergeIdListWithDocList - Sample merged doc type: ${firstDoc.type}`);
      if (firstDoc.options) {
        console.log(`[DEBUG] mergeIdListWithDocList - Sample merged doc has ${firstDoc.options.length} options`);
      } else {
        console.log(`[DEBUG] mergeIdListWithDocList - Sample merged doc has no options`);
      }
    }
